...
// ── (f) Real Ark settlement ──────────────────────────────────────────────────
// BLOCKED: the current one-step referee API creates its MuSig2 nonces only
// after receiving the provider partial that already depends on those nonces.
// Gate 2 ends at authenticated Accept plus fail-closed Provider rejection.
// See extro/docs/release/GATE3-ARK-PROTOCOL-MIGRATION.md for the required
// prepare/lock/initiate protocol and the acceptance matrix.
test.fixme(
	'(f) BLOCKED: signed Ark prepare handshake and genuine wallet ciphertext are required',
	async () => {}
);
