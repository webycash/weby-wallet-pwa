<script lang="ts">
	...
	import { orderbook, selectPair, evaluatePair, pairReleaseAvailability, type TradingPair } from '$lib/modules/webycash-exchange';
...
<div class="space-y-4">
	...
	<div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
		{#each orderbook.pairs as p (pairKey(p))}
			...
			{@const rel = pairReleaseAvailability(p.base, p.quote)}
			<button
				onclick={() => rel.available && open(p)}
				disabled={!rel.available}
				title={!rel.available ? rel.message : undefined}
				data-testid={rel.available ? 'market-available' : 'market-unavailable'}
				class="w-full flex items-center justify-between gap-3 px-4 py-3 rounded-2xl transition-all duration-200
					{rel.available ? 'bg-muted/30 hover:bg-muted/50 active:scale-[0.99]' : 'bg-muted/20 opacity-60 cursor-not-allowed'}
